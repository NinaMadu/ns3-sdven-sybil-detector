#!/bin/bash
# =============================================================================
# type2_pct_sweep_mode5.sh — attacker-percentage sweep for attack type 2
# (Insider Direct Simultaneous) under solution_mode=5 (MODE_FULL).
#
# Companion to the mode-4 baseline run at
#   sybil-attack/datasets/type2_pct_sweep_20260803_151014/pct20_s1
# Every flag below is identical to that run except --solution_mode and the
# full-mode-only knobs, so the two are directly comparable.
#
# Why not sequential_all6: this sweeps ONE attack type, so --datasetMode is
# omitted and --sybil_attack_type=2 drives the legacy per-type scheduler
# (sybil_attacks.h case ATTACK_INSIDER_DIRECT_SIMULTANEOUS). Consequences:
...
#
# --llmDetectInterval=5 (default 10) so a 30 s run gets 6 scoring windows.
#
# The C++ launches the Python daemon itself (llm_realtime_detection.h
# LaunchDaemon), so no daemon needs to be started by hand.
#
# Usage:
#   bash sybil-attack/commands/type2_pct_sweep_mode5.sh
#   PCTS="20 40" bash sybil-attack/commands/type2_pct_sweep_mode5.sh
# =============================================================================
...
DETECT_INTERVAL=${DETECT_INTERVAL:-5}

STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_NAME=${OUT_NAME:-"type2_mode5_sweep_${STAMP}"}
...
echo "=========================================================="
echo " Type-2 x attacker-% sweep, MODE_FULL (solution_mode=5)"
echo " Percentages : $PCTS"
echo " Seed        : $SEED"
echo " simTime     : ${SIM_TIME}s   detectInterval: ${DETECT_INTERVAL}s"
...
for pct in $PCTS; do
    ...
    ( cd "$SIM_DIR" && "$SIM_BIN" \
        ...
        --simTime=${SIM_TIME} \
        --beaconInterval=0.1 \
        --llmDetectInterval=${DETECT_INTERVAL} \
        --ipfsPublish=false \
        --seed=${SEED} \
        --runId=${run_id} \
        --outputDir=${run_rel} \
