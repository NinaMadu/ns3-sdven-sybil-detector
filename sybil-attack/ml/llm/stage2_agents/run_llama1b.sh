#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Stage-2 (3-agent consensus) re-run on Llama-3.2-1B-Instruct.
#
# This is a NEW, self-contained driver. It does NOT touch any existing code,
# adapters, scorecards, or preds. It only:
#   - trains 3 LoRA agents on Llama-3.2-1B  -> adapters/llama1b_a{1,2,3}
#   - runs consensus_eval with that base    -> scorecards_llama1b/, new preds file
#
# The existing Qwen2.5-1.5B run (adapters/qwen1_5b_a*, scorecards/, preds/
# stage2_agent_preds.parquet) is left completely untouched, so results stay
# directly comparable.
#
# Same frozen recipe as the Qwen run (r=8, alpha=16, 2 epochs, batch 8,
# grad-accum 2, max-len 1024 -> the train_lora_generic.py defaults), so the
# only thing that changes between the two runs is the base LLM.
#
# Usage:
#   bash run_llama1b.sh          # train 3 agents, then consensus eval
#   bash run_llama1b.sh eval     # skip training, just re-run consensus eval
# ---------------------------------------------------------------------------
set -euo pipefail
